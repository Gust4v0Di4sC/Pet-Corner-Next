...
export function AnimaisRecordsSection({
  ...
  const listGroup = useMemo(() => buildDogListGroup(items), [items]);
  ...
  const deleteController = useRecordDeleteController({
    recordsById,
    recordItemsById,
    entityLabel: dogFormConfig.entityLabel,
    successMessage: dogFormConfig.deleteSuccessMessage,
    onDelete: remove,
    notifyAdminAction,
  });
