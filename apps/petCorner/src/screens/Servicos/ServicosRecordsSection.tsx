...
import {
  buildServiceListGroup,
  buildServicePayload,
  getServiceFormData,
  serviceFormConfig,
} from "./servicos.records";

type Props = {
  items: PetService[];
  isLoading: boolean;
  create: (payload: Omit<PetService, "id">) => Promise<void>;
  update: (recordId: string, payload: Omit<PetService, "id">) => Promise<void>;
  remove: (recordId: string) => Promise<void>;
};

export function ServicosRecordsSection({
  items,
  isLoading,
  create,
  update,
  ...
  const listGroup = useMemo(() => buildServiceListGroup(items), [items]);
  ...
  const deleteController = useRecordDeleteController({
    recordsById,
    recordItemsById,
    entityLabel: serviceFormConfig.entityLabel,
    successMessage: serviceFormConfig.deleteSuccessMessage,
    onDelete: remove,
    notifyAdminAction,
  });
  ...
  return (
    <section className="record-management">
      ...
      <RecordCreateButton
        ariaLabel="Adicionar novo servico"
